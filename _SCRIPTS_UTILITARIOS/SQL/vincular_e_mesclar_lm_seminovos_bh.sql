-- ============================================================
-- Script: Mesclar cadastros e vincular lançamentos LM SEMINOVOS BH
-- Mesclar: "LM SEMINOVOS - BH" e "Loja Patio Contagem LM BH" → "LM SEMINOVOS BH"
-- Termos de busca nos lançamentos: LM BH
-- Data: 2026-01-08
-- ============================================================
...
-- 1.1 Buscar todos os cadastros com "LM" e "BH" ou "SEMINOVOS"
SELECT
  id,
  nome,
  tipo,
  ...
  created_at
FROM pessoas
WHERE LOWER(nome) LIKE '%lm%seminovos%'
   OR LOWER(nome) LIKE '%lm%bh%'
   OR LOWER(nome) LIKE '%patio%contagem%'
ORDER BY created_at;
...
-- 2.1 Contar lançamentos com LM BH
SELECT
  COUNT(*) as total_lancamentos,
  SUM(valor_total) as valor_total
FROM financeiro_lancamentos
WHERE LOWER(descricao) LIKE '%lm%bh%'
   OR LOWER(descricao) LIKE '%lm seminovos%';
...
WHERE LOWER(l.descricao) LIKE '%lm%bh%'
   OR LOWER(l.descricao) LIKE '%lm seminovos%'
...
DECLARE
  ...
  rec RECORD;
BEGIN
  -- Buscar o cadastro PRINCIPAL "LM SEMINOVOS BH" (sem hífen)
  SELECT id INTO v_principal_id
  FROM pessoas
  WHERE LOWER(nome) = 'lm seminovos bh'
     OR LOWER(nome) LIKE 'lm seminovos bh%'
  ORDER BY created_at ASC
  LIMIT 1;

  -- Se não encontrou, tentar com hífen
  IF v_principal_id IS NULL THEN
    SELECT id INTO v_principal_id
    FROM pessoas
    WHERE LOWER(nome) LIKE '%lm seminovos%bh%'
    ORDER BY created_at ASC
    LIMIT 1;
  END IF;

  -- Se ainda não encontrou, pegar qualquer um com LM e BH
  IF v_principal_id IS NULL THEN
    SELECT id INTO v_principal_id
    FROM pessoas
    WHERE LOWER(nome) LIKE '%lm%'
      AND (LOWER(nome) LIKE '%bh%' OR LOWER(nome) LIKE '%seminovos%')
    ORDER BY created_at ASC
    LIMIT 1;
  END IF;

  IF v_principal_id IS NULL THEN
    RAISE EXCEPTION 'Cliente LM SEMINOVOS BH não encontrado!';
  END IF;
  ...
  FOR rec IN
    ...
    WHERE (
        LOWER(nome) LIKE '%lm%seminovos%'
        OR LOWER(nome) LIKE '%lm%bh%'
        OR LOWER(nome) LIKE '%patio%contagem%lm%'
      )
  ...
  LOOP
    ...
    SET
      ativo = false,
      nome = nome || ' [MESCLADO → LM SEMINOVOS BH]',
      updated_at = NOW()
  ...
  -- Vincular todos os lançamentos com "LM BH" na descrição
  UPDATE financeiro_lancamentos
  SET
    cliente_centro_custo_id = v_principal_id,
    updated_at = NOW()
  WHERE (
      LOWER(descricao) LIKE '%lm%bh%'
      OR LOWER(descricao) LIKE '%lm seminovos%'
    )
  ...
  RAISE NOTICE '% lançamentos vinculados ao cliente LM SEMINOVOS BH', v_count_lancamentos;
END $$;
