 ...
 * Design Mobile-First com navegação inferior estilo app
...
const menuItems = [
  ...
  {
    label: "Materiais",
    href: "/colaborador/materiais",
    icon: Package,
  },
  {
    label: "Diário",
    href: "/colaborador/diario-obra",
    icon: HardHat,
  },
];
...
export default function ColaboradorLayout() {
  ...
  const carregarStatusGeral = useCallback(async () => {
    ...
    try {
      ...
      const somaProgresso = projetosAtivos.reduce(
        (acc, p) => acc + (p.projeto?.progresso || 0),
        0
      );
  ...
  const saudacao = useMemo(() => {
    ...
    if (hora >= 5 && hora < 12) {
      return { texto: "Bom dia", icon: Sunrise, cor: "text-amber-400" };
    ...
    <div className="min-h-screen bg-gray-50 pb-20 md:pb-0">
      ...
      <header className="bg-gradient-to-r from-slate-800 via-slate-900 to-slate-800 px-4 py-4 md:p-6 lg:p-8 shadow-lg relative">
        ...
        <div className="max-w-7xl mx-auto">
          {/* Layout Mobile: Avatar + Saudação na mesma linha */}
            ...
            <div className="flex items-center gap-4 flex-shrink-0">
              {usuario?.cargo && (
                <div className="bg-slate-700/50 rounded-xl p-4 border border-slate-600/50 min-w-[160px]">
                  <p className="text-xs text-gray-400 uppercase tracking-wider mb-1">Cargo</p>
                  <p className="text-lg font-semibold text-white">{usuario.cargo}</p>
                  {usuario?.empresa && (
              ...
              <div className="bg-slate-700/50 rounded-xl p-4 border border-slate-600/50 min-w-[180px]">
                ...
                <p className="text-3xl font-bold text-orange-400">{statusGeral.progresso}%</p>
            ...
            <div className="flex items-center gap-3 bg-slate-700/30 rounded-lg px-3 py-2 border border-slate-600/30">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-bold text-orange-400">{statusGeral.progresso}%</span>
                </div>
