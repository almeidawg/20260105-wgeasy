...
export default function AprovacoesPendentes({ clienteId, contratoId, onAprovar }: Props) {
  ...
  async function carregarItens() {
    try {
      ...
      if (!orcError && orcamentos) {
        // Carregar itens de cada orçamento
  ...
  async function handleAprovarMaterial(pedidoId: string) {
    try {
      ...
      alert("Pedido de materiais aprovado!");
      setObservacoesAprovacao("");
      await carregarItens();
      onAprovar?.();
    } catch (error: any) {
    ...
    return (
      <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="font-semibold text-gray-900">Aprovações Pendentes</h3>
        </div>
      ...
      <div className="divide-y divide-gray-100">
        {itensFiltrados.map((item) => {
          ...
          if (item.tipo === "orcamento") {
            return renderOrcamento(item as OrcamentoCliente, isExpandido);
          } else {
            return renderMaterial(item as PedidoMaterialCliente, isExpandido);
          }
        })}
      </div>
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md">
            <div className="px-6 py-4 border-b border-gray-200">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-gray-900">Rejeitar Item</h3>
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tfoot className="bg-gray-50">
                      <tr>
                        ...
                        <td className="px-4 py-3 text-lg font-bold text-gray-900 text-right">{formatarValor(orcamento.valor_total)}</td>
                      </tr>
                    </tfoot>
                  </table>
                </div>
