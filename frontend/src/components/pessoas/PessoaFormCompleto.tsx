...
import { FolderPlus, ExternalLink, Loader2, FolderCheck } from "lucide-react";
...
export function PessoaFormCompleto({ tipo, onSubmit, onCancel, initialData }: PessoaFormCompletoProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-4xl mx-auto space-y-6 font-poppins">
      ...
      {tipo === "CLIENTE" && (
        <div className="bg-white p-6 rounded-lg border shadow-sm">
          <h2 className="text-lg font-semibold mb-4 text-gray-800">Google Drive</h2>
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium mb-1">Link da pasta do cliente</label>
              ...
              <p className="text-xs text-gray-500 mt-1">
                Cole o link da pasta ou clique em &quot;Gerar Pasta&quot; para criar automaticamente.
              </p>
            </div>

            {/* Botão para gerar pasta automaticamente */}
            {isBackendConfigured() && (
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      setForm((prev: any) => ({ ...prev, drive_link: folder.link }));
                  ...
                  className="inline-flex items-center gap-2 px-4 py-2 bg-[#F25C26] text-white rounded-md hover:bg-[#d94d1f] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {criandoPastaDrive ? (
                    <>
