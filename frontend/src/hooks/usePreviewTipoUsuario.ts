...
// Usado na Planta do Sistema para validação de permissões
...
export function PreviewTipoUsuarioProvider({ children }: { children: ReactNode }) {
  ...
  const startPreview = useCallback((tipo: TipoUsuario) => {
    ...
    console.log("[Preview] Iniciando visualização como:", tipo);
  }, []);

  const stopPreview = useCallback(() => {
    setPreviewTipo(null);
    sessionStorage.removeItem(STORAGE_KEY);
    console.log("[Preview] Finalizando visualização");
  }, []);
...
export function usePreviewTipoUsuario(): PreviewTipoUsuarioContextType {
  ...
  // Se não estiver dentro do provider, retornar estado padrão
  if (!context) {
    return {
      previewTipo: null,
      isPreviewMode: false,
