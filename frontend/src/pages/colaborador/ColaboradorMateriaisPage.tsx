 ...
 * Fluxo: Pedido → Verifica Pricelist → Orçamento → Aprovação → Compras
 */
...
export default function ColaboradorMateriaisPage() {
  ...
  const buscarNoPricelist = useCallback(async (termo: string, index: number) => {
    ...
    try {
      ...
      setSugestoesPricelist((prev) => ({ ...prev, [index]: data || [] }));
  ...
  const pedidosFiltrados = pedidos.filter((p) => {
    const matchBusca =
      ...
      p.projeto_nome?.toLowerCase().includes(busca.toLowerCase());
                ...
                <div className="relative mt-1">
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      // Limpar seleção se o usuário editar
                      if (clienteSelecionado) {
                        setClienteSelecionado("");
                      }
                    }}
                    ...
                    <div key={index} className="space-y-2 pb-3 border-b last:border-b-0 last:pb-0">
                          ...
                          <div className="relative">
                            ...
                            {loadingBusca[index] && (
                              <div className="absolute right-2.5 top-1/2 -translate-y-1/2">
                                <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-[#F25C26]" />
                              </div>
                            )}
                          </div>
                      ...
                      {!item.pricelist_id && item.nome.length >= 2 && !loadingBusca[index] && sugestoesPricelist[index]?.length === 0 && (
                        <p className="text-xs text-amber-600 pl-1">
                          Item não encontrado no pricelist. Será incluído manualmente.
                        </p>
                      )}
                    </div>
                  ))}
