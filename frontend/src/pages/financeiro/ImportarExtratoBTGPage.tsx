/**
 * PÁGINA DE IMPORTAÇÃO - EXTRATO BTG PACTUAL
...
// Dados de importação inline (gerados a partir do PDF)
const DADOS_IMPORTACAO = {
  ...
  centros_custo: {
    "MO DIRETA/PEDREIRO": "Mão de Obra - Pedreiro",
    "MO DIRETA/ELETRICA": "Mão de Obra - Elétrica",
    "MO DIRETA/HIDRAULICA": "Mão de Obra - Hidráulica",
    "MO DIRETA/GESSO": "Mão de Obra - Gesso/Drywall",
    "MO DIRETA/PINTURA": "Mão de Obra - Pintura",
    "MO DIRETA/AJUDANTE": "Mão de Obra - Ajudante",
    "MO DIRETA/FAZ TUDO": "Mão de Obra - Faz Tudo",
    "MO DIRETA": "Mão de Obra Direta",
    "MATERIAIS/CONSTRUCAO": "Materiais - Construção",
    "SERVICOS": "Serviços",
    "ADMINISTRATIVO/TELEFONIA": "Administrativo - Telefonia",
    "DESPESA/ALIMENTACAO": "Despesas - Alimentação",
    "RECEITA/CLIENTE": "Receita - Cliente",
    "INTERCOMPANY": "Intercompany",
    "PENDENTE": "Pendente Classificação"
...
export default function ImportarExtratoBTGPage() {
  ...
  async function getOrCreatePessoa(nome: string): Promise<string | null> {
    ...
    if (error) {
      addLog(`Aviso: Não foi possível criar pessoa ${nome}`);
  ...
  async function importarLancamento(lancamento: LancamentoImportado): Promise<boolean> {
    try {
      ...
      const { error } = await supabase
        ...
        .insert({
          ...
          observacoes: lancamento.status === "PENDENTE_VINCULO"
            ? `Importado BTG - PENDENTE VALIDAÇÃO - ${lancamento.cliente_obra}`
      ...
      addLog(`Exceção #${lancamento.id}: ${String(err)}`);
      return false;
    }
  }
  ...
  async function executarImportacao() {
    if (!jsonData.trim()) {
      alert('Cole os dados JSON no campo de texto');
      return;
    }
    ...
    if (!Array.isArray(lancamentos) || lancamentos.length === 0) {
      alert('Nenhum lançamento encontrado no JSON');
      return;
    }
    ...
    addLog(`Iniciando importação de ${lancamentos.length} lançamentos...`);
    ...
    addLog(`Importação concluída: ${ok} sucesso, ${fail} falhas`);
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <h1 className="text-2xl font-bold mb-4">Importar Extrato BTG Pactual</h1>
      ...
      {logs.length > 0 && (
        <div className="mt-6">
          <h3 className="font-medium mb-2">Log de Importação:</h3>
