...
export default function DashboardPage() {
      ...
      try {
        ...
        const [
          ...
          supabase.from("cronograma_tarefas")
            .select("id, status")
            .gte("data_inicio", new Date(hoje.getFullYear(), hoje.getMonth(), 1).toISOString().split("T")[0]),
        ]);
        ...
        const contratosConcluidos = contratos.filter(c =>
          c.status === 'concluido' || c.status === 'finalizado' || c.status === 'encerrado'
        ).length;
        ...
        const tarefasConcluidas = tarefas.filter((t: any) =>
          t.status === 'concluido' || t.status === 'concluída' || t.status === 'done'
        ).length;
        ...
        setMetrics({
          ...
          contratosConcluidos,
          valorContratos,
          nucleoArquitetura: nucleoArq || Math.floor(contratosAtivosCount * 0.35),
          nucleoEngenharia: nucleoEng || Math.floor(contratosAtivosCount * 0.40),
          nucleoMarcenaria: nucleoMarc || Math.floor(contratosAtivosCount * 0.25),
          despesaDesigner,
          despesaArquitetura,
          despesaEngenharia,
          despesaMarcenaria,
  ...
  const statusGeral = useMemo(() => {
    ...
    return Math.round((metrics.tarefasConcluidas / metrics.tarefasTotais) * 100);
  ...
  const despesasNucleoPercent = useMemo(() => {
    const total = metrics.despesaDesigner + metrics.despesaArquitetura +
                  metrics.despesaEngenharia + metrics.despesaMarcenaria;
    if (total === 0) return { designer: 0, arquitetura: 0, engenharia: 0, marcenaria: 0 };
    return {
  ...
  }, [metrics.despesaDesigner, metrics.despesaArquitetura, metrics.despesaEngenharia, metrics.despesaMarcenaria]);
        ...
        <header className="bg-gradient-to-r from-slate-800 via-slate-900 to-slate-800 rounded-2xl py-4 px-5 lg:py-5 lg:px-6 shadow-lg">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center gap-4">
            ...
            <div className="flex items-center gap-3 flex-shrink-0">
              ...
              <div className="relative">
                <div className="w-14 h-14 lg:w-16 lg:h-16 bg-gradient-to-br from-orange-400 to-orange-600 rounded-full flex items-center justify-center shadow-lg overflow-hidden">
                  {(usuario?.avatar_url || usuario?.foto_url) ? (
                    <img src={usuario.avatar_url || usuario.foto_url || ''} alt={usuario?.nome || 'Usuário'} className="w-full h-full object-cover" />
                  ) : (
                    <span className="text-2xl font-bold text-white">{usuario?.nome?.charAt(0) || 'W'}</span>
                  )}
                </div>
              ...
              <div>
                <p className="text-[10px] text-gray-400 uppercase tracking-wider mb-0.5">WG Easy · Dashboard Executivo</p>
            ...
            <div className="bg-slate-700/50 rounded-lg p-3 border border-slate-600/50 min-w-[160px] flex-shrink-0">
              <p className="text-[10px] text-gray-400 uppercase tracking-wider mb-0.5">Status Geral</p>
              <p className="text-2xl font-bold text-white">{statusGeral}%</p>
              <p className="text-[10px] text-gray-400">
                {metrics.tarefasConcluidas}/{metrics.tarefasTotais} tarefas do mês
              </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="text-lg font-semibold text-gray-900">Contratos ativos do dia</h3>
                <p className="text-sm text-gray-500">Mini cards com avatar, acesso rápido ao contato do cliente e endereço da obra</p>
              </div>
              <div className="text-right">
                <p className="text-xs text-gray-400 uppercase tracking-wider">Clientes Ativos</p>
                <p className="text-2xl font-bold text-gray-900">{metrics.contratosAtivos}</p>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-3 mt-4">
              {clientesAtivos.length > 0 ? (
                clientesAtivos.map((cliente) => {
                  ...
                  const bgClass = nucleo === 'arquitetura' ? 'bg-orange-100 ring-orange-200' :
                                  nucleo === 'engenharia' ? 'bg-blue-100 ring-blue-200' : 'bg-amber-100 ring-amber-200';
                  const textClass = nucleo === 'arquitetura' ? 'text-orange-500' :
                                    nucleo === 'engenharia' ? 'text-blue-500' : 'text-amber-500';
                  return (
                    <div
                    ...
                    >
                      <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-2 overflow-hidden ring-2 ${bgClass}`}>
                      ...
                      <p className="text-xs font-medium text-gray-700 text-center truncate w-full" title={cliente.nome}>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-semibold text-gray-900">Checklist Diário</h3>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-semibold text-gray-900">Dashboard Financeiro</h3>
                <p className="text-sm text-gray-500">Visão executiva consolidada</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <div className="flex bg-gray-100 rounded-lg p-1">
                <button type="button" className="px-3 py-1.5 text-xs font-medium text-gray-600 hover:text-gray-900 rounded-md transition-colors">3 Meses</button>
                <button type="button" className="px-3 py-1.5 text-xs font-medium text-gray-600 hover:text-gray-900 rounded-md transition-colors">6 Meses</button>
                <button type="button" className="px-3 py-1.5 text-xs font-medium text-gray-600 hover:text-gray-900 rounded-md transition-colors">Este Ano</button>
                <button type="button" className="px-3 py-1.5 text-xs font-medium text-gray-600 hover:text-gray-900 rounded-md transition-colors">12 Meses</button>
                <button type="button" className="px-3 py-1.5 text-xs font-medium bg-slate-800 text-white rounded-md transition-colors">Todos</button>
              </div>
              <button type="button" onClick={() => navigate('/financeiro')} className="p-2 hover:bg-gray-100 rounded-lg transition-colors" title="Ver detalhes">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
            ...
            <div className="p-4 rounded-xl bg-gradient-to-br from-orange-500 to-orange-600 text-white relative overflow-hidden">
              ...
              <p className="text-[10px] uppercase tracking-wider text-white/80 mb-1">Saldo Atual</p>
              <p className="text-lg font-bold">
                {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(metrics.receitaMes - metrics.despesaMes)}
              </p>
            ...
            <div className="p-4 rounded-xl bg-gradient-to-br from-orange-400 to-orange-500 text-white relative overflow-hidden">
              ...
              <p className="text-[10px] uppercase tracking-wider text-white/80 mb-1">Receitas</p>
              <p className="text-lg font-bold">
                {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(metrics.receitaMes)}
              </p>
              <p className="text-[10px] text-white/70 mt-1">↗ {metrics.contratosAtivos} contratos</p>
            </div>
            ...
            <div className="p-4 rounded-xl bg-gradient-to-br from-amber-400 to-amber-500 text-white relative overflow-hidden">
              ...
              <p className="text-[10px] uppercase tracking-wider text-white/80 mb-1">Custos Totais</p>
              <p className="text-lg font-bold">
                {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(metrics.despesaMes)}
              </p>
            ...
            <div className="p-4 rounded-xl bg-gray-50 border border-gray-200 relative overflow-hidden">
              ...
              <p className="text-[10px] uppercase tracking-wider text-gray-500 mb-1">Contratos Ativos</p>
              <p className="text-lg font-bold text-gray-900">{metrics.contratosAtivos}</p>
              <p className="text-[10px] text-gray-500 mt-1">
                {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL', notation: 'compact' }).format(metrics.receitaMes / (metrics.contratosAtivos || 1))}
              </p>
            </div>
            ...
            <div className="p-4 rounded-xl bg-gray-50 border border-gray-200 relative overflow-hidden">
              ...
              <p className="text-[10px] uppercase tracking-wider text-gray-500 mb-1">Concluídos</p>
              <p className="text-lg font-bold text-gray-900">{metrics.contratosConcluidos}</p>
              <p className="text-[10px] text-gray-500 mt-1">Contratos finalizados</p>
            </div>
          </div>
          ...
          <div className="h-[280px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={dadosMensais}>
                <defs>
                  <linearGradient id="colorEntradasFin" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#10b981" stopOpacity={0.4}/>
                    <stop offset="95%" stopColor="#10b981" stopOpacity={0.05}/>
                  </linearGradient>
                  <linearGradient id="colorSaidasFin" x1="0" y1="0" x2="0" y2="1">
                ...
                <YAxis stroke="#9ca3af" fontSize={11} tickLine={false} axisLine={false} tickFormatter={(value) => `${(value/1000).toFixed(0)}k`} />
                <Tooltip
                ...
                />
                <Area type="monotone" dataKey="receitas" stroke="#10b981" strokeWidth={2} fillOpacity={1} fill="url(#colorEntradasFin)" name="receitas" />
                <Area type="monotone" dataKey="despesas" stroke="#f97316" strokeWidth={2} fillOpacity={1} fill="url(#colorSaidasFin)" name="despesas" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 h-6 bg-gray-100 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{ width: `${despesasNucleoPercent.designer || 0}%` }}
                    />
                  </div>
                  <span className="text-xs text-gray-500 w-16 text-right">
                    {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL', notation: 'compact' }).format(metrics.despesaDesigner)}
                  </span>
                </div>
                {/* Arquitetura */}
                <div className="flex items-center gap-4">
                  <span className="text-sm text-gray-600 w-24">arquitetura</span>
                  <div className="flex-1 h-6 bg-gray-100 rounded-full overflow-hidden">
                    <div
                      className="h-full bg-orange-400 rounded-full transition-all duration-500"
                      style={{ width: `${despesasNucleoPercent.arquitetura || 0}%` }}
                    />
                  </div>
                  <span className="text-xs text-gray-500 w-16 text-right">
                    {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL', notation: 'compact' }).format(metrics.despesaArquitetura)}
                  </span>
                </div>
                {/* Engenharia */}
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 h-6 bg-gray-100 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{ width: `${despesasNucleoPercent.engenharia || 0}%` }}
                    />
                  </div>
                  <span className="text-xs text-gray-500 w-16 text-right">
                    {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL', notation: 'compact' }).format(metrics.despesaEngenharia)}
                  </span>
                </div>
                {/* Marcenaria */}
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 h-6 bg-gray-100 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{ width: `${despesasNucleoPercent.marcenaria || 0}%` }}
                    />
                  </div>
                  <span className="text-xs text-gray-500 w-16 text-right">
                    {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL', notation: 'compact' }).format(metrics.despesaMarcenaria)}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-semibold text-gray-900">Propostas Comerciais</h3>
                    <p className="text-xs text-gray-500">Conversão e pipeline</p>
                  </div>
                </div>
                <button
                  type="button"
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="text-center p-4 bg-amber-50 rounded-xl border border-amber-100">
                  <p className="text-3xl font-bold text-amber-600">{metrics.propostasAbertas}</p>
                  <p className="text-xs text-gray-500 mt-1">Aguardando</p>
                </div>
                <div className="text-center p-4 bg-emerald-50 rounded-xl border border-emerald-100">
                  <p className="text-3xl font-bold text-emerald-600">{metrics.propostasAprovadas}</p>
              ...
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">Controle pessoal do CEO</p>
                  </div>
                </div>
                <button
                  type="button"
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                    ...
                    <div className="p-4 rounded-xl bg-gradient-to-br from-amber-400 to-amber-300">
                      ...
                      <p className={`text-xl font-bold ${dadosPessoais.balanco_mes >= 0 ? 'text-white' : 'text-red-600'}`}>
              ...
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-semibold text-gray-900">Você foi Mencionado</h3>
                      <p className="text-xs text-gray-500">
              ...
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">{alertas.length} itens requerem ação</p>
                  </div>
                </div>
                    ...
                    >
                      ...
                      {alerta.acao && (
                        <span className={`text-xs font-medium ${
                          ...
                          alerta.tipo === 'atencao' ? 'text-amber-600' : 'text-blue-600'
                        }`}>
                          {alerta.acao} →
                        </span>
                      )}
