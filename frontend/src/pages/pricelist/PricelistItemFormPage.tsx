...
const pricelistItemSchema = z.object({
  ...
  producao_diaria: z
    .number()
    .min(0, "Produção/dia deve ser maior ou igual a 0")
    .optional(),
...
export default function PricelistItemFormPage() {
    ...
    if (id) {
      async function carregar() {
        try {
          ...
          if (!item?.id) {
            setErroCarregamento("Item não encontrado ou erro ao carregar.");
            return;
          }
  ...
  if (erroCarregamento) {
    return (
      <div className="p-6 flex items-center justify-center min-h-screen">
        <div className="text-center">
          <p className="text-lg text-red-600 font-bold">{erroCarregamento}</p>
          <button
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              ...
              criar uma descrição didática automaticamente
            </p>
          </div>
                ...
                <input
                  ...
                  className={`w-full pl-10 pr-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#F25C26] focus:border-transparent ${
                    errors.preco
                      ? "border-red-500"
                      : calculoPreco
                      ? "border-green-300 bg-green-50 cursor-not-allowed font-semibold text-green-700"
                      : "border-gray-300"
                  }`}
                />
              </div>
              ...
              <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-yellow-700 mt-0.5">
                    ...
                    regras de precificação do núcleo{" "}
                    <strong>
                      {nucleos.find((n) => n.id === nucleoSelecionado)?.nome}
                    </strong>
