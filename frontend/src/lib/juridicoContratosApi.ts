 ...
 * - Parcelas de contratos (criação em lote, pagamentos)
 ...
 * @param intervaloMeses - Intervalo entre parcelas em meses (padrão: 1)
 */
...
export async function criarAditivo(
  ...
  // Calcular nova previsão de término se houver dias adicionais
...
export async function buscarContratosComAlertas(): Promise<ContratoAlerta[]> {
  ...
  const { data: contratos, error } = await supabase
    ...
    .select(`
      ...
      status
    `)
    .in("status", ["assinado", "em_execucao"])
...
export async function obterEstatisticasJuridico(): Promise<EstatisticasJuridico> {
  ...
  // Contratos ativos (assinados ou em execução)
  ...
  const { data: valores } = await supabase
    ...
    .in("status", ["assinado", "em_execucao", "concluido"]);
