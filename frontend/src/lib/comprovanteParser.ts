...
export interface DadosComprovante {
  ...
  confianca: number; // 0-100 - quão confiável é o parsing
...
function extrairRecebedor(texto: string): {
  ...
  for (let i = 0; i < linhas.length; i++) {
    ...
    if (linhaLower === 'instituição' || linhaLower === 'banco') {
...
function extrairIdTransacao(texto: string): string | null {
  // Padrões de ID de transação
  const padroes = [
    // PIX: E + números
    /(?:ID|Transação|ID\/Transação)[:\s]*\n?\s*(E\d{20,})/i,
    /\b(E\d{20,})\b/,
    // TED/DOC: números longos
    /(?:autenticação|comprovante|protocolo)[:\s]*\n?\s*(\d{15,})/i,
  ];
