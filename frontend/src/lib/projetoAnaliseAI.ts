...
async function chamarOpenAITexto(
  ...
  if (!apiKey) {
    throw new Error(
      "Configure VITE_OPENAI_API_KEY ou inicie o backend para usar o provedor OpenAI."
    );
  }
...
async function chamarClaudeVision(
  ...
  if (!apiKey) {
    throw new Error(
      "Configure VITE_ANTHROPIC_API_KEY para usar o provedor Anthropic."
    );
  }
...
export async function processarArquivoProjeto(arquivo: File): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    if (!tiposAceitos.includes(arquivo.type)) {
      reject(
        new Error(
          "Tipo de arquivo não suportado. Use JPEG, PNG, GIF ou WebP. PDFs devem ser convertidos para imagem primeiro."
        )
      );
      return;
    }
...
`;
      ...
      if (tentativa > 0 && tentativa <= RETRY_CONFIG.promptsCorrecao.length) {
        ...
        console.log(
          `[analisarEscopoComIA] Aplicando correção de prompt #${tentativa}`
        );
      }
      ...
      if (!foiTruncada) {
        ...
        if (!terminaComFechamento || numAbre > numFecha) {
          foiTruncada = true;
          console.warn(
            `[analisarEscopoComIA] ATENÇÃO: JSON parece truncado (chaves: ${numAbre} abertas, ${numFecha} fechadas)`
          );
        }
      }
      ...
      if (foiTruncada) {
        console.warn(
          "[analisarEscopoComIA] ATENÇÃO: Resposta foi TRUNCADA - tentando recuperar serviços parciais"
        );
      }
      ...
      if (resultado.servicos.length === 0) {
        ...
        throw new Error(
          "A análise não retornou nenhum serviço. Verifique se o texto do escopo está correto."
        );
      }
...
function parsearRespostaEscopo(
  ...
  if (!resultado.success) {
    console.log(
      "[parsearRespostaEscopo] Parsing falhou, tentando extração direta de serviços..."
    );
    servicosExtraidosDiretamente = extrairServicosDeJsonTruncado(resposta);

    if (servicosExtraidosDiretamente.length > 0) {
      console.log(
        `[parsearRespostaEscopo] Extração direta recuperou ${servicosExtraidosDiretamente.length} serviços!`
      );
  ...
  console.log(
    `[parsearRespostaEscopo] servicosRaw (antes da validação): ${
  ...
  console.log(
    `[parsearRespostaEscopo] servicos (após validação): ${servicos.length} válidos`
  );
...
function inferirTipoAmbiente(nome: string): string {
  ...
  if (nomeLower.includes("circulação") || nomeLower.includes("corredor"))
    return "corredor";
