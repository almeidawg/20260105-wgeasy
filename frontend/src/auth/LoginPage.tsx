...
export default function LoginPage() {
  ...
  // Ao acessar /login, limpamos qualquer sessão pré-existente para evitar reaproveitar perfis antigos
  useEffect(() => {
    const limparSessao = async () => {
      try {
        const { data, error } = await supabase.auth.getSession();
        // Só tenta fazer logout se realmente houver uma sessão válida
        if (!error && data?.session) {
          await supabase.auth.signOut();
        }
      } catch (err) {
        // Ignora erros silenciosamente - não há sessão para limpar
        console.debug("[Login] Nenhuma sessão para limpar");
      }
    };
    limparSessao();
  }, []);
